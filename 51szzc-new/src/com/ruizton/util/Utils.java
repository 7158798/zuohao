...
public class Utils {
	...
	public static boolean saveFile(String dir, String fileName,
		...
		flag = oss.PostObject(fileName, dir+"/"+fileName) ;
		//文件上传完成后，删除服务器上的存储的文件
	...
	public static boolean saveFile(String fileName,
		...
		try{
			OSSPostObject oss = new OSSPostObject() ;
			flag = oss.PostObject(fileName, inputStream) ;
		}catch (Exception e){
...
//		return ( (long)(value*Math.pow(10, scale)) ) /Math.pow(10.0, scale) ;
	...
	public static boolean isDevEnvironment(){
		String domain = ConstantMap.getString("Domain");
		if(StringUtils.isNotBlank(domain)){
			if(domain.contains("test.51szzc.com:8080") || domain.contains("130.252.100")){
				return true;
			}
		}
		return false;
	...
	public static void main(String[] args) {
		System.out.print(getDoubleForAccount(2));
	}
