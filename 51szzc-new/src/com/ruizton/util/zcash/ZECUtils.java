package com.ruizton.util.zcash;
 ...
 */
public class ZECUtils implements IWalletUtil {
    ...
    public ZECUtils(BTCMessage btcMessage){
    ...
    public JSONObject getNewaddressForAdmin() throws Exception {
        String s = main("getnewaddress", "[]");
        JSONObject json = JSONObject.fromObject(s);
        return json;
    }

    public String getNewaddressValueForAdmin() throws Exception {
        String result = null;
        if(PASSWORD != null && PASSWORD.trim().length() >0){
            walletpassphrase(30);
        }
        JSONObject s = getNewaddressForAdmin();
        if(PASSWORD != null && PASSWORD.trim().length() >0){
            walletlock();
        }
        if(s.containsKey("result")){
    ...
    public JSONObject sendtoaddress(String address,double account,String comment) throws Exception {
        ...
        String s = main("sendtoaddress", "[\""+address+"\","+account+"]");
        if(PASSWORD != null && PASSWORD.trim().length() >0){
            walletlock();
        }
        JSONObject json = JSONObject.fromObject(s);
...
//		settxfee(ffees);
    ...
    public static void main(String[] args) throws Exception {
       ...
       /*btcMessage.setIP("130.252.101.153") ;
        ...
        btcMessage.setSECRET_KEY("szzc2017") ;*/
        ZECUtils zecUtils = new ZECUtils(btcMessage);
        /**/double value = zecUtils.getbalanceValue();
        System.out.println("value " + value);
        /*String address = zecUtils.getNewaddressValueForAdmin();
        System.out.println("address " + address);*/
        /*String test = zecUtils.sendtoaddressValue("tmBgdXM7NazqNDsaBtY7rUHKZhtwNYVPEYE",2,0,"test");
        System.out.println("txid " + test);*/
        /*double a1 = 0.000085;
        double a2 = 0.034572;
        double all = a1 + a2;
        double ff = 0;
        ...
        ff = ff + t1;
        all = all - t1;
        all = all + 0.001 + 0.020220;
        ff = ff - 0.001 - 0.020220;
