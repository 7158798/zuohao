...
import com.ruizton.util.TaoBaoIpHelper;
...
public class FrontUserService extends BaseService {
	...
	public boolean saveRegister(Fuser fuser) throws Exception{
		try {
			...
			List<Fvirtualcointype> fvirtualcointypes = (List)this.constantMap.get("allCoins");
			...
			for (Fvirtualcointype fvirtualcointype : fvirtualcointypes) {
				...
				if (fvirtualcointype.isFisEth()){
					// 重新获取货币对象,主帐户
					fvirtualcointype = this.fvirtualcointypeDAO.findById(fvirtualcointype.getFid());
				}

				Fpool fpool = this.fpoolDAO.getOneFpool(fvirtualcointype) ;
				if(fpool == null) {
					LOG.i(this, "注册时，获取币种可用地址信息为空");
					throw new RuntimeException("注册时，获取币种可用地址信息为空") ;
				}
...
//			this.fbankinfoWithdrawDAO.save(fbankinfoWithdraw) ;
	...
	public Fuser updateCheckLogin(Fuser fuser,String ip,boolean ismail,boolean isLog){
		Fuser flag = null ;
		try{
...
//			}
			...
			if(flag!=null && isLog){
				...
				Flog flog = new Flog() ;
				//根據ip獲取地址
				flog.setAddress(TaoBaoIpHelper.getAddress(ip));
	...
	public Fuser queryById(int id) {
		return this.fuserDAO.queryById(id);
	}

	//原生态查询用户信息、带外键表主键
	public Fuser queryUserScoreById(int id) {
		return this.fuserDAO.queryUserScoreById(id);
	}

	//查询用户信息  级联查询
	public Fuser findByIdCase(int id){
		Fuser fuser = this.fuserDAO.findById(id) ;
		if(fuser != null) {
	...
	public void updateFUser(Fuser fuser,HttpSession session,int logType,String ip){
		...
		if(logType >0){//操作记录。无记录填负数
			Flog flog = new Flog() ;
			if(logType==LogTypeEnum.User_LOGIN){
				flog.setAddress(TaoBaoIpHelper.getAddress(ip));
			}
			flog.setFcreateTime(Utils.getTimestamp()) ;
	...
	public void updateFuser(Fuser fuser,Fvalidatekyc fvalidatekyc){
		try{
			if(fuser != null){
				this.fuserDAO.attachDirty(fuser) ;
			}
			if(fvalidatekyc != null){
				if(fvalidatekyc.getId() == 0){
					this.fvalidatekycDAO.save(fvalidatekyc); ;
				}else{
					this.fvalidatekycDAO.attachDirty(fvalidatekyc) ;
				}
