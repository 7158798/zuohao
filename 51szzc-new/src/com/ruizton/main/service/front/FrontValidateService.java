...
public class FrontValidateService extends BaseService{
	...
	//用户注册邮件验证
	public boolean updateMailValidate(int uid,String uuid,String ip) throws Exception{
		boolean flag = false ;
		try{
			...
			flog.setFkey3(ip) ;
			flog.setFtype(LogTypeEnum.User_BIND_EMAIL) ;
	...
	public boolean saveSendFindPasswordMail(String ip,Fuser fuser,String email,HttpServletRequest request){
		...
		try {
			...
			validateemails.setFtitle(this.getSystemArgs(ConstantKeys.WEB_NAME)+"找回登录密码邮件") ;
			validateemails.setFcontent(
					...
					.replace("#englishName#", this.getSystemArgs(ConstantKeys.WEB_NAME))) ;
	...
	public int getLimitCount(String ip,int type){
		...
		if(list.size()==0){
			...
			if(Utils.getTimestamp().getTime()- fcountlimit.getFcreateTime().getTime()<1L*30*60*1000){
	...
	public void updateLimitCount(String ip,int type){
		...
		if(list.size()==0){
			...
			Fcountlimit fcountlimit = list.get(0) ;
			if(Utils.getTimestamp().getTime()- fcountlimit.getFcreateTime().getTime()<1L*30*60*1000L){
