...
public class FarticleDAO extends HibernateDaoSupport  {
	...
	public List<Farticle> findFarticle(int farticletype,int firstResult,int maxResult){
		...
		try {
			String queryString = "from Farticle where farticletype.fid=? order by fisding desc, fLastModifyDate desc";
	         Query queryObject = getSession().createQuery(queryString);
	         queryObject.setCacheable(true) ;
	...
	public List<Farticle> findFarticleByTypeOut(int farticletype,int firstResult,int maxResult){
		log.debug("findFarticle all Farticle instances");
		try {
			String queryString = "from Farticle where farticletype.fid=? and fstatus in (2,4)  order by fisding desc, fLastModifyDate desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setCacheable(true) ;
			queryObject.setParameter(0, farticletype) ;
			queryObject.setFirstResult(firstResult) ;
	...
	public List<Farticle> findOutArticle(int firstResult,int maxResult){
		log.debug("findOutArticle Farticle instances");
		try {
			String queryString = "from Farticle where fisout = 1 and fstatus in (2,4) order by flastModifyDate desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setCacheable(true) ;
			queryObject.setFirstResult(firstResult) ;
			queryObject.setMaxResults(maxResult) ;
			...
			log.error("findOutArticle failed", re);
			throw re;
		}
	}
	...
	public List<Farticle> findHostArticle(int firstResult,int maxResult){
		log.debug("findFarticle all Farticle instances");
		try {
			String queryString = "from Farticle where fisding = 1 and fstatus in (2,4) order by flastModifyDate desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setCacheable(true) ;
			queryObject.setFirstResult(firstResult) ;
			queryObject.setMaxResults(maxResult) ;
	...
	public int findFarticleCount(int farticletype){
		...
		try {
			String queryString = "select count(*) from Farticle where farticletype.fid=? and fstatus in (2,4) order by id desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setCacheable(true) ;
			queryObject.setParameter(0, farticletype) ;
