...
public class FvirtualcointypeDAO extends HibernateDaoSupport {
	...
	public void saveOrUpdate(Fvirtualcointype persistentInstance) {
		log.debug("updating Fvirtualcointype instance");
		try {
	 ...
	 * @return
	 */
	public int updateAutoWithConfig(AutoWithConfigVo vo) {
		int result = 0;
		if(vo == null || StringUtils.isBlank(vo.getAuto_day_count()) || StringUtils.isBlank(vo.getAuto_day_limit()) || StringUtils.isBlank(vo.getAuto_single_limit())) {
			return result;
		}

		String sql = "update Fvirtualcointype set auto_day_count= ?,auto_day_limit = ?, auto_single_limit = ? where fid = ?";
		Query queryObject = getSession().createQuery(sql);
		queryObject.setParameter(0, Integer.valueOf(vo.getAuto_day_count()));
		queryObject.setParameter(1, new BigDecimal(vo.getAuto_day_limit()));
		queryObject.setParameter(2, new BigDecimal(vo.getAuto_single_limit()));
		queryObject.setParameter(3, Integer.valueOf(vo.getFid()));
