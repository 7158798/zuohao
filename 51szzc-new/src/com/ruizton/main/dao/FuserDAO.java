...
public class FuserDAO extends HibernateDaoSupport  {
    ...
    //原生态查询用户信息
    public Fuser queryById(int id) {
		Fuser fuser = null;
		StringBuffer sql = new StringBuffer();
		sql.append(" select fid, floginName, ifnull(fRealName,''), kyc_level, fStatus, fisTelephoneBind, ");
		sql.append(" fIsMailValidate, fIsTelValidate, fGoogleValidate,fhasRealValidate,ifnull(fTelephone,''), ");
		sql.append(" ifnull(fsafebind, 0), ifnull(fEmail,''), ifnull(favatarUrl,''), ifnull(ftradePassword,'') ");
		sql.append(" from fuser where fid = ? ");
		...
		fuser.setFid(Integer.valueOf(o[0].toString()));
		fuser.setFloginName(o[1].toString());
		fuser.setFrealName(o[2].toString());
		fuser.setKyclevel(Integer.valueOf(o[3].toString()));
		fuser.setFstatus(Integer.valueOf(o[4].toString()));
		fuser.setFisTelephoneBind(Boolean.valueOf(o[5].toString()));
		fuser.setFisMailValidate(Boolean.valueOf(o[6].toString()));
		fuser.setFisTelValidate(Boolean.valueOf(o[7].toString()));
		fuser.setFgoogleValidate(Boolean.valueOf(o[8].toString()));
		fuser.setFhasRealValidate(Boolean.valueOf(o[9].toString()));
		fuser.setFtelephone(o[10].toString());
		fuser.setFsafebind(Boolean.valueOf(o[11].toString()));
		fuser.setFemail(o[12].toString());
		fuser.setFavatarUrl(o[13].toString());
		fuser.setFtradePassword(o[14].toString());
		return fuser;
	}


	//原生态查询用户信息、带外键表主键
	public Fuser queryUserScoreById(int id) {
		Fuser fuser = null;
		StringBuffer sql = new StringBuffer();
		sql.append(" select a.fid, floginName, ifnull(fRealName,''), kyc_level, fStatus, fisTelephoneBind, ");
		sql.append(" fIsMailValidate, fIsTelValidate, fGoogleValidate,fhasRealValidate,ifnull(fTelephone,''), ");
		sql.append(" ifnull(fsafebind, 0), ifnull(fEmail,''), ifnull(favatarUrl,''),ifnull(fscoreid, 0), ifnull(b.flevel,0), ");
		sql.append(" ifnull(a.ftradePassword,'') ");
		sql.append(" from fuser a ");
		sql.append(" inner join fscore b on a.fscoreid = b.fid");
		...
		fscore.setFid(Integer.valueOf(o[14].toString()));
		fscore.setFlevel(Integer.valueOf(o[15].toString()));
		fuser.setFscore(fscore);
		fuser.setFtradePassword(o[16].toString());
		return fuser;
	}
