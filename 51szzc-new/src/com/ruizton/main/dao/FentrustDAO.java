...
public class FentrustDAO extends HibernateDaoSupport {
	...
	public List<Fentrustlog> findNewLatestDeal(int coinTypeId, int max_result){
		log.debug("findLatestSuccessDeal all Fentrust instances");
		try {
			String date = DateHelper.getHistoryDate(-5);
			String queryString = "from Fentrustlog where  ftrademapping.fid=?  and fEntrustType = 0  and fCreateTime > '" + date + "'   order by fid desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, coinTypeId) ;
	...
	public List<Map> getTotalQty(int fentrustType,String entrustFilter) {
		...
		if (StringUtils.isNotEmpty(entrustFilter)){
			sql.append("and a.FUs_fId not in(" + entrustFilter + ") \n");
		}
	...
	public List<Map> getTotalQty(int fentrustType,boolean isToday,String entrustFilter) {
		List<Map> all = new ArrayList();
		StringBuffer sql = new StringBuffer();
	 ...
	 * 获取每日的成交量
	 * @param userId		用户id
	 * @param fentrustType	交易类型
	 * @return
	 */
	public BigDecimal getTotalQtyByTrade(int userId,int fentrustType){

		StringBuffer sql = new StringBuffer();
		sql.append("select IFNULL(SUM(a.fCount - a.fleftCount),0) FROM ");
		sql.append(" fentrust a WHERE a.FUs_fId = " + userId);
		sql.append(" AND a.fEntrustType = " + fentrustType);
		sql.append(" AND DATE_FORMAT(a.fCreateTime, '%Y-%d-%m') = DATE_FORMAT(SYSDATE(), '%Y-%d-%m')");
