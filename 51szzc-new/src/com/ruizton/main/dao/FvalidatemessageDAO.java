...
public class FvalidatemessageDAO extends HibernateDaoSupport {
	...
	public List<Fvalidatemessage> list(int firstResult, int maxResults,
		...
		List<Fvalidatemessage> list = null;
		log.debug("finding Fvalidatemessage instance with filter");
		try {
			String queryString = "from Fvalidatemessage "+filter;
			Query queryObject = getSession().createQuery(queryString);
			...
			log.error("find by Fvalidatemessage name failed", re);
			throw re;
		}
		return list;
	}
