...
public class KlinePeriodData {
	...
	private void readData(){
		...
		for (Ftrademapping ftrademapping : ftrademappings) {
			...
			String[] jsonStrings = new String[17] ;
	...
	public synchronized void generateJson(int id){
		...
		for (Map.Entry<Integer, TreeSet<Fperiod>> entry : map.entrySet()) {
			...
			for (Fperiod fperiod : fperiods) {
				...
				if(entry.getKey() == 0){
					if(size <= 1440){
						appStr.append("["+(fperiod.getFtime().getTime())
								+","+new BigDecimal(String.valueOf(fperiod.getFkai())).setScale(6, BigDecimal.ROUND_HALF_UP)
								+","+new BigDecimal(String.valueOf(fperiod.getFgao())).setScale(6, BigDecimal.ROUND_HALF_UP)
								+","+new BigDecimal(String.valueOf(fperiod.getFdi())).setScale(6, BigDecimal.ROUND_HALF_UP)
								+","+new BigDecimal(String.valueOf(fperiod.getFshou())).setScale(6, BigDecimal.ROUND_HALF_UP)
			...
			if(entry.getKey() == 0){
				this.setJsonString(id, 13, appStr.toString());
			}else if(entry.getKey() == 2){
				this.setJsonString(id, 14, appStr.toString());
			}else if(entry.getKey() == 4){
				this.setJsonString(id, 15, appStr.toString());
			}else if(entry.getKey() == 5){
				this.setJsonString(id, 16, appStr.toString());
			}
		}
	}
