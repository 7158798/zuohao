		...
		<thead>
			<tr>
				...
				<th width="60" orderField="ftotal"
					<c:if test='${param.orderField == "ftotal" }'> class="${param.orderDirection}"  </c:if>>总数量1</th>
		...
		<tbody>
			<c:forEach items="${virtualwalletList}" var="virtualwallet"
				...
				<tr target="sid_user" rel="${virtualwallet.fid}">
					...
					<td><fmt:formatNumber value="${virtualwallet.ftotal}" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="6"/></td>
					<td><fmt:formatNumber value="${virtualwallet.ffrozen}" pattern="##.######" maxIntegerDigits="15" maxFractionDigits="6"/></td>
