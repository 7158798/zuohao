		...
		<tbody>
			<c:forEach items="${virtualCapitaloperationList}"
				...
				<tr target="sid_user" rel="${virtualCapitaloperation.fid}">
					...
					<td>
						<c:choose>
							<c:when test="${not empty virtualCapitaloperation.mailLinkUUID && virtualCapitaloperation.fstatus == 3 }">
								系统处理成功
							</c:when>
							<c:otherwise>
					...
					<td><fmt:formatNumber value="${virtualCapitaloperation.ffees}" pattern="##.######" maxIntegerDigits="20" maxFractionDigits="8"/></td>
