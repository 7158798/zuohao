...
var trade = {
    ...
    NumVerify : function(tradeType) {
        ...
        if (tradeType == 0) {
            ...
            //判断是否非法输入
            var price_falg = util.checkNumber(userCnyBalance);
            var count_flag = util.checkNumber(tradebuyAmount)
            if (!price_falg || !count_flag) {
            ...
            //判断是否非法输入
            var price_falg = util.checkNumber(usersellCnyBalance);
            var count_flag = util.checkNumber(tradesellAmount)
            if (!price_falg || !count_flag) {
                util.showerrortips(errorele, language["price_or_num_error"]);
                return;
            } else {
...
//	},
	...
	submitTradeBtcForm : function(tradeType, flag) {
		//买卖交易
		var limited = 0;  //是否是市价交易，0限价单，1市价单
		$("#limitedType").val(0) ;
		...
		if (tradeType == 0) { //买入
			//是否是市价单
			limited = document.getElementById("tradebuyType").value;
            //买入数量
			var tradeAmount = document.getElementById("tradebuyamount").value;
			//买入价格
			var tradeCnyPrice = limited == 0 ? document.getElementById("tradebuyprice").value : document.getElementById("tradebuymoney").value;


		} else {
			limited = document.getElementById("tradesellType").value;
			var tradeAmount = limited == 0 ? document.getElementById("tradesellamount").value : document.getElementById("tradesellnum").value;
			var tradeCnyPrice = limited == 0 ? document.getElementById("tradesellprice").value : document.getElementById("tradesellnum").value;
		}

		//重新隐藏域赋值
        $("#limitedType").val(limited) ;

		var isAMT = util.checkNumber(tradeAmount);
		var isPrice = util.checkNumber(tradeCnyPrice);

		//限价单时：价格和数量都不能为空
		if(limited == 0 && (!isAMT || !isPrice)){
			util.showerrortips(errorele, language["price_or_num_error"]);
			return;
		}
		
		if(limited == 1 && !isPrice) {
			if(tradeType == 0) {
				util.showerrortips(errorele, language["buy_amount_error"]);
				return;
			} else {
				util.showerrortips(errorele, language["sell_num_error"]);
				return;
			}
		}
		console.log(tradeCnyPrice)
        ...
        if(limited == 1 && tradeType == 0) {  //限价单买入，则交易数量为0
            tradeAmount = 0;
        }
	...
	antoTurnover : function(price, money, mtcNum, tradeType) {
		if (tradeType == 0) {
			console.log(util.moneyformat(price, coinCount1))
			console.log(price, coinCount1)
...
//			console.log(tradeTurnover);
	...
	autoRefresh : function() {
		...
		$.get(url, function(data) {
			var buy_str = i18nMsg('buy');  //买
            var sell_str = i18nMsg('sell');  //卖
			$.each(data.buys, function(key, value) {
				...
				var buyele = $("#buy" + key);
				if (buyele.length == 0) {
					$("#buybox").append('<li id="buy' + key + '"  class="list-group-item clearfix buysellmap" data-type="1" data-money="' + util.moneyformat(Number(value.price), coinCount1) + '" data-num="' + util.moneyformat(Number(value.amount), coinCount2) + '">' + '<span class="col-xs-2 redtips padding-clear" style="width:20%;">' + buy_str + (key + 1) + '</span>' + '<span class="col-xs-5 text-right padding-clear" style="width:40%;">' + util.moneyformat(Number(value.price), coinCount1) + '</span>' + '<span class="col-xs-5 redtips text-right padding-clear" style="width:40%;">' + util.moneyformat(Number(value.amount), coinCount2) + '</span></li>');
				} else {
			...
			$.each(data.sells, function(key, value) {
				...
				if (sellele.length == 0) {
					$("#sellbox").prepend('<li id="sell' + key + '"  class="list-group-item clearfix buysellmap" data-type="0" data-money="' + util.moneyformat(Number(value.price), coinCount1) + '" data-num="' + util.moneyformat(Number(value.amount), coinCount2) + '">' + '<span class="col-xs-2 greentips padding-clear" style="width:20%;">' + sell_str + (key + 1) + '</span>' + '<span class="col-xs-5 text-right padding-clear" style="width:40%;">' + util.moneyformat(Number(value.price), coinCount1) + '</span>' + '<span class="col-xs-5 greentips text-right padding-clear" style="width:40%;">' + util.moneyformat(Number(value.amount), coinCount2) + '</span></li>');
				} else {
...
$(function() {
	...
	$('#tradebuymoney').on('keypress', function(event) {
		return util.VerifyKeypress(this, event, coinCount1);
	});
	$('#tradesellnum').on('keypress', function(event) {
		return util.VerifyKeypress(this, event, coinCount2);
	});
	...
	//切换交易方式
	$('#tradebuyType').on('change', function() {
		var type = $(this).val();

		//$('#tradebuyprice,#tradebuyamount').val('');
		$('#tradebuyamount').trigger('keyup');
		$('#buyType').val(type);

		$('.buyarea').removeClass('active');
		
		if(type == 0) {
			$('.buyarea').eq(0).addClass('active');
		} else {
			$('.buyarea').eq(1).addClass('active');
		}
	});
	$('#tradesellType').on('change', function() {
		//$('#tradesellprice,#tradesellamount').val('');
		$('#tradesellamount').trigger('keyup');

		var type = $(this).val();
		$('#sellType').val(type);

		$('.sellarea').removeClass('active');
		
		if(type == 0) {
			$('.sellarea').eq(0).addClass('active');
		} else {
			$('.sellarea').eq(1).addClass('active');
		}
	});
