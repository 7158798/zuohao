var assets = {
	...
	detelAlipay : function (account) {
		util.layerConfirm(language["withdraw.error.tips.10"],
			function(result) {
				$('#alertTips').modal('hide');
				var url = "/user/delAlipayBind.html?random=" + Math.round(100 * Math.random()),
					param = {
						account: account
					};
				$.post(url, param,
	...
	addAlipy: function () {
		...
		$.post('/user/addAlipayBind.html?random=' + Math.round(Math.random() * 100), {
			...
			if(data.code == 0) {
				var msg_content = i18nMsg("bind_success");
				$('#addAlipy-confirm').text(msg_content);
				location.reload(); 
			} else {
				var msg_content = i18nMsg("bind_fail");
				util.showerrortips("alipyerrortips", data.msg || msg_content);
			}
...
$(function() {
	...
	$(".alipay-del").on("click", function() {
		assets.detelAlipay($(this).attr('data-account'));
	});
