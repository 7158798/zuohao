var util = {
	...
	VerifyKeypress : function(ele, event, decimal) {
		...
		if (decimal == 0 && keyCode == 46) {
			...
			event.returnValue = false;
		} else if (this.getCursortPosition(ele) > 0 && ele.value[0] == 0 && keyCode != 46 && ele.value.length < 2 && keyCode != 8) {
			event.returnValue = false;
    ...
    layerConfirm: function(a, b) {
        options = {
            ...
            maxWidth: 200,
            title: language["comm.error.tips.37"],
            btn: [i18nMsg("ok_desc"), i18nMsg("cancel_desc1")]
        },
        b = b ||
        function(a) {},
