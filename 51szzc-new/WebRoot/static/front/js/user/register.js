var reg = {
	...
	checkPassword : function() {
		...
		if (desc != "") {
			util.showerrortips("register-error-password", desc);
			return false;
		} else {
			util.hideerrortips("register-error-password");
		}
		return true;
	},
	...
	checkRegUserNameNoJquery : function() {
		...
		if (desc != "") {
			if(regType == 0) {
				util.showerrortips("register-error-phone", desc);
			} else {
				util.showerrortips("register-error-email", desc);
			}
			return false;
		} else {
	...
	regSubmit : function() {
		...
		if (flag == true) {
			...
			if (regType == 0) {
				...
				if (regPhoneCode == "") {
					util.showerrortips("register-error-msg", language["comm.error.tips.60"]);
					return;
				} else {
					util.hideerrortips("register-error-msg");
				}
			}
			var regEmailCode = 0;
			if (regType == 1) {
				...
				if (regEmailCode == "") {
					util.showerrortips("register-error-emailcode", language["comm.error.tips.138"]);
					return;
				} else {
					util.hideerrortips("register-error-emailcode");
				}
			}
			var intro_user = document.getElementById("intro_user").value;
			// var intro_user = '';
