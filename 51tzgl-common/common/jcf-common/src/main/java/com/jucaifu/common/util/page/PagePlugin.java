...
public class PagePlugin implements Interceptor {
        ...
        if (ivk.getTarget() instanceof RoutingStatementHandler) {
            ...
            if (mappedStatement.getId().matches(pageSqlId)) { //拦截需要分页的SQL
                ...
                if (parameterObject == null) {
                    ...
                    String countSql = SqlParser.getSmartCountSql(sql); //记录统计
     ...
     * @throws SQLException
     */
    private void setParameters(PreparedStatement ps, MappedStatement mappedStatement, BoundSql boundSql, Object
        ...
        if (parameterMappings != null) {
            ...
            for (int i = 0; i < parameterMappings.size(); i++) {
                ...
                if (parameterMapping.getMode() != ParameterMode.OUT) {
                    ...
                    } else if (propertyName.startsWith(ForEachSqlNode.ITEM_PREFIX) && boundSql.hasAdditionalParameter
                            (prop.getName())) {
                        value = boundSql.getAdditionalParameter(prop.getName());
                        if (value != null) {
                            value = configuration.newMetaObject(value).getValue(propertyName.substring(prop.getName()
                                    .length()));
                        }
                    } else {
    ...
    public void setProperties(Properties p) {
        ...
        if (ValueHelper.checkStringIsEmpty(dialect)) {
            LOG.d(this, "dialect property is not found!");
        }

        pageSqlId = p.getProperty("pageSqlId");
        if (ValueHelper.checkStringIsEmpty(pageSqlId)) {
            LOG.d(this, "pageSqlId property is not found!");
        }
    }
